...
| Atributo | Escenario | Origen | Artefacto | Respuesta |
|:---|:---|:---|:---|:---|
| Escalabilidad | La plataforma experimenta un crecimiento del 200% en usuarios activos durante los primeros 3 meses. | Incremento de demanda | API Gateway / Microservicios | El sistema permite escalar horizontalmente cada microservicio de forma independiente, desplegando nuevas instancias en menos de 5 minutos sin afectar la disponibilidad. |
| Confiabilidad | Un usuario realiza una recarga de saldo y la transacción se interrumpe por un error en el gateway de pagos. | Fallo en el procesamiento de pago | Payments Service | El sistema implementa un mecanismo de reintentos con backoff exponencial (3 intentos) y registra la transacción en un estado "pendiente" para su posterior reconciliación. |
| Portabilidad | El equipo decide migrar los microservicios de Azure a AWS por temas de costo. | Cambio de proveedor cloud | Todos los microservicios | El sistema utiliza contenedores Docker y orquestación con Kubernetes, permitiendo la migración a cualquier proveedor cloud en menos de 2 días sin modificar el código fuente. |
